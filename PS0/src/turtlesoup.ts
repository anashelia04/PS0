...
export function drawSquare(turtle: Turtle, sideLength: number): void {
  for (let i = 0; i < 4; i++) {
    turtle.forward(sideLength); // Move forward by the side length
    turtle.turn(90); // Turn 90 degrees to form a square
  }
}
...
export function chordLength(radius: number, angleInDegrees: number): number {
  ...
  return 2 * radius * Math.sin(angleInRadians / 2); // Chord length formula
}
...
export function drawApproximateCircle(
  ...
  const angle = 360 / numSides; // Angle for each segment
  const sideLength = chordLength(radius, angle); // Length of each segment

  for (let i = 0; i < numSides; i++) {
    turtle.forward(sideLength); // Move forward by the segment length
    turtle.turn(angle); // Turn by the segment angle
  }
}
...
export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    const distanceToMove = distance(p1, p2);
    const angleToTurn = Math.atan2(dy, dx) * (180 / Math.PI);

    instructions.push(`turn ${angleToTurn}`);
